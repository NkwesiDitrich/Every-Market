        ...
        <Stack>
            ...
            <Stack component={Paper} elevation={2}>
                <Stack component={'form'} width={is500?"95vw":'30rem'} p={'1rem'} rowGap={'1rem'} noValidate onSubmit={handleSubmit(handleResetPassword)}>
                        ...
                        <Stack rowGap={'.3rem'}>
                            <Typography variant='h4' fontWeight={600}>{t('auth.reset.title')}</Typography>
                            <Typography color={'GrayText'}>{t('auth.reset.subtitle')}</Typography>
                        </Stack>
                        ...
                        <Stack rowGap={'.5rem'}>
                            <MotionConfig whileHover={{y:-2}}>
                                ...
                                <motion.div>
                                    <TextField type='password' fullWidth sx={{mt:1}} {...register("password",{required:t('auth.reset.passwordRequired'),pattern:{value:/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{8,}$/gm,message:t('auth.reset.passwordRules')}})} placeholder={t('auth.reset.passwordPlaceholder')}/>
                                    {errors.password && <FormHelperText sx={{mt:1}} error>{errors.password.message}</FormHelperText>}
                                </motion.div>
                                
                                <motion.div>
                                    <TextField type='password' fullWidth sx={{mt:1}} {...register("confirmPassword",{required:t('auth.reset.confirmRequired'),validate:(value,formValues)=>value===formValues.password || t('auth.reset.confirmMismatch')})} placeholder={t('auth.reset.confirmPlaceholder')}/>
                        ...
                        <motion.div whileHover={{scale:1.020}} whileTap={{scale:1}}>
                            <LoadingButton sx={{height:"2.5rem"}} fullWidth loading={status==='pending'} type='submit' variant='contained'>{t('auth.reset.button')}</LoadingButton>
                        </motion.div>
                </Stack>
            </Stack>
