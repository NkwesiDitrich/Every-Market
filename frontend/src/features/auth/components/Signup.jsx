...

      {
        ...
        <Stack flex={1} justifyContent={'center'} alignItems={'center'}>
              ...
              <Stack flexDirection={'row'} justifyContent={'center'} alignItems={'center'}>
                  <Stack rowGap={'.4rem'}>
                    <Typography variant='h2' sx={{wordBreak:"break-word"}} fontWeight={600}>Sign Up</Typography>
                    <Typography alignSelf={'flex-end'} color={'GrayText'} variant='body2'>- Create your account</Typography>
                  </Stack>
                ...
                <Stack mt={4} spacing={2} width={is480?"95vw":'28rem'} component={'form'} noValidate onSubmit={handleSubmit(handleSignup)}>
                    ...
                    <MotionConfig whileHover={{y:-5}}>
                      ...
                      <motion.div>
                        <TextField fullWidth {...register("name",{required:'Full name is required'})} placeholder="Full Name"/>
                      ...
                      <motion.div>
                        <TextField type='password' fullWidth {...register("password",{required:'Password is required',pattern:{value:/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{8,}$/gm,message:'Min 8 chars, 1 uppercase, 1 lowercase, 1 number'}})} placeholder="Password"/>
                        {errors.password && <FormHelperText error>{errors.password.message}</FormHelperText>}
                      </motion.div>
                      
                      <motion.div>
                        <TextField type='password' fullWidth {...register("confirmPassword",{required:'Please confirm your password',validate:(value,fromValues)=>value===fromValues.password || 'Passwords do not match'})} placeholder="Confirm Password"/>
                    ...
                    <motion.div whileHover={{scale:1.020}} whileTap={{scale:1}}>
                      <LoadingButton sx={{height:'2.5rem'}} fullWidth loading={status==='pending'} type='submit' variant='contained'>Sign Up</LoadingButton>
                    </motion.div>
