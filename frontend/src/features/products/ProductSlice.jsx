...
const productSlice=createSlice({
    ...
    reducers:{
        ...
        addToComparison:(state, action)=>{
            if(state.comparisonList.length < 4 && !state.comparisonList.find(p => p._id === action.payload._id)) {
                state.comparisonList.push(action.payload);
            }
        },
        removeFromComparison:(state, action)=>{
            state.comparisonList = state.comparisonList.filter(p => p._id !== action.payload);
        },
...
export const selectComparisonList=(state)=>state.ProductSlice.comparisonList
export const selectSearchQuery=(state)=>state.ProductSlice.searchQuery

// exporting actions
export const {clearProductSuccessMessage,clearProductErrors,clearSelectedProduct,resetProductStatus,resetProductUpdateStatus,resetProductAddStatus,toggleFilters,resetProductFetchStatus, addToComparison, removeFromComparison, clearComparison, setSearchQuery}=productSlice.actions
