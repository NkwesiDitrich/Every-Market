...
import { createWishlistItem, deleteWishlistItemById, fetchWishlistByUserId, updateWishlistItemById, toggleWishlistPublic, fetchPublicWishlist } from './WishlistApi'
...
export const toggleWishlistPublicAsync = createAsyncThunk('wishlist/toggleWishlistPublicAsync', async (isPublic) => {
    const response = await toggleWishlistPublic(isPublic)
    return response
})

export const fetchPublicWishlistAsync = createAsyncThunk('wishlist/fetchPublicWishlistAsync', async (userId) => {
    const response = await fetchPublicWishlist(userId)
    return response
})
...
const wishlistSlice=createSlice({
    ...
    extraReducers:(builder)=>{
        builder
            ...
            .addCase(toggleWishlistPublicAsync.pending, (state) => {
                state.wishlistPublicStatus = 'pending'
            })
            .addCase(toggleWishlistPublicAsync.fulfilled, (state, action) => {
                state.wishlistPublicStatus = 'fulfilled'
                state.isWishlistPublic = action.payload.isPublic
                // Sync all local items' isPublic flag
                state.items.forEach(item => { item.isPublic = action.payload.isPublic })
            })
            .addCase(toggleWishlistPublicAsync.rejected, (state, action) => {
                state.wishlistPublicStatus = 'rejected'
                state.errors = action.error
            })

            .addCase(fetchPublicWishlistAsync.pending, (state) => {
                state.wishlistFetchStatus = 'pending'
            })
            .addCase(fetchPublicWishlistAsync.fulfilled, (state, action) => {
                state.wishlistFetchStatus = 'fulfilled'
                state.publicWishlist = action.payload
            })
            .addCase(fetchPublicWishlistAsync.rejected, (state, action) => {
                state.wishlistFetchStatus = 'rejected'
                state.errors = action.error
            })
    }
...
export const selectIsWishlistPublic=(state)=>state.WishlistSlice.isWishlistPublic
export const selectPublicWishlist=(state)=>state.WishlistSlice.publicWishlist
export const selectWishlistPublicStatus=(state)=>state.WishlistSlice.wishlistPublicStatus
